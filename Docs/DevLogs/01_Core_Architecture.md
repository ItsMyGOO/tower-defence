...
## AudioManager / EffectsManager 双管理器单一职责拆分（EventBus 解耦触发）
原 AudioManager 同时管理音频与视觉特效违反单一职责，已按功能严格拆分为两个独立 Node：AudioManager 仅负责 BGM 与 SFX 播放及音频节点回收，EffectsManager 仅负责 CPUParticles2D / GPUParticles2D 等视觉特效的实例化与零闪烁销毁；两个管理器在 Level_01.tscn 与 AudioTest.tscn 的 Systems 节点下各自独立挂载，互相无引用无依赖，后续新增震屏、飘字等特效只需扩展 EffectsManager 而不触及音频模块，反之亦然。
AudioManager 统一管理局内全部背景音乐与一次性音效，所有触发完全基于 EventBus 订阅：_Ready 时监听 OnTowerBuilt / OnEnemyKilled / OnEnemyReachedEnd / OnGameOver 四个事件，收到事件后立即播放对应 SFX，自身不反向引用 TowerManager、Enemy、EconomyManager 等任何业务模块，新增成就或过场音效只需在该类内追加订阅即可，零侵入现有战斗流程；BGM 采用单个常驻 AudioStreamPlayer 子节点（命名 BGMPlayer）维护，GameOver 时通过 Tween 线性降低 VolumeDb 至静音后停止，SFX 则按需动态实例化并监听 Finished 信号自动 QueueFree，_ExitTree 时再对活动集合兜底强制释放，确保节点零泄漏。
EffectsManager 独立承担击杀视觉特效生命周期：通过 PackedScene EnemyDeathEffectScene 导出配置接收特效预制体，订阅同一 EventBus.OnEnemyKilled 事件并以 deathPosition 参数在世界坐标定位 Instantiate 后挂入自身节点树；为彻底消除粒子最后一帧与销毁回调间的帧竞争闪烁，销毁策略严格为 CPU 粒子提前 20ms 触发回调 → 立即 Visible=false 切断当前帧之后的所有渲染批次 → 1ms 零等待 Tween 在下一帧才 QueueFree，GPU 粒子监听 Finished 信号后走同一隐藏+下一帧销毁路径，未知类型默认 1.98s 触发同一流程；两管理器拆分后 AudioManager 子节点基准值恒定为 1（仅 BGMPlayer）、EffectsManager 基准值恒定为 0，通过双计数对比可快速定位音频或特效哪一侧存在回收异常。

## 关卡结束/中途 → 主菜单的双向闭环出口
Meta Loop 除「正向推进（解锁→下一关）外，还必须提供「反向退出」闭环出口，否则玩家战败或中途放弃战斗会被迫 Alt+F4。本模块在 GameOverPanel 结算面板新增 MainMenuButton（返回主菜单）按钮，胜利战败均可见，点击时按 Paused=false → SceneManager.LoadMainMenu() 的顺序回到 Meta Loop 起点，LoadMainMenu 同时将 CurrentLevelIndex 重置为 0，表示正式脱离战斗上下文；局中途的出口则新增独立的 PauseMenuPanel 暂停菜单面板，由 HUDView 动态实例化并挂载为子节点，玩家通过 HUD 右上角的 PauseButton 或按 ESC 键（HUDView._UnhandledInput 捕获，统一调用 PauseMenu.TogglePause() 切换暂停与显隐，暂停菜单面板提供 5 条出口（继续/重开/返回选关/返回主菜单/退出，其中返回主菜单按钮直接调用 SceneManager.LoadMainMenu() 重置 CurrentLevelIndex 并回到主菜单）。HUD 订阅 EventBus.OnGameOver 时会调用 PauseMenu.ForceClose() 确保暂停遮罩，Level_01 / Level_02 两关 _Input 顶部添加 GetTree().Paused 判断后短路，避免暂停后仍可点 Slot 触发建塔残留状态。

## SceneManager 全局场景流转与 Meta Loop 架构
SceneManager 配置为 Godot AutoLoad 单例（project.godot [autoload] 节注册），统一维护跨场景跳转的唯一入口：LoadScene / LoadMainMenu / LoadLevelSelect / LoadLevel / LoadNextLevel 五个方法分层封装 ChangeSceneToFile，所有出口调用前显式 GetTree().Paused = false 保证下一场景从干净非暂停时钟启动。关卡解锁系统基于 Godot ConfigFile 写入 OS.GetUserDataDir()/TowerDefence_Save.cfg 本地持久化：MaxUnlockedLevel 默认 1，_Ready 时从 ConfigFile 读取恢复；UnlockNextLevel 严格幂等，仅当 CurrentLevelIndex == MaxUnlockedLevel 时才自增并写入 ConfigFile 并 Save() 落盘，重复调用或非最高关卡通关均不会重复推进进度；CurrentLevelIndex 在每次 LoadLevel 成功后同步更新，LoadNextLevel 据此计算目标关卡、校验解锁状态与场景文件存在性、最终回落至选关界面保证流程始终有出口。LoadMainMenu 在跳转前将 CurrentLevelIndex 重置为 0，作为战斗上下文到菜单上下文的硬边界重置动作。
...
GameOverPanel 新增 NextLevelButton 与 LevelSelectButton 两个流转出口，并补充 MainMenuButton 返回主菜单的直接出口，将原有 QuitButton 顺序下移形成 6 按钮纵向 VBox。收到 EventBus.OnGameOver(true) 胜利事件时立即调用 SceneManager.Instance?.UnlockNextLevel() 推进解锁，随后根据 nextIndex、场景文件存在性、MaxUnlockedLevel 三者综合判定 NextLevelButton 是否可点击：所有条件满足时显示"➡️ 下一关"，否则显示"🏆 已是最后一关"并置灰禁用；战败时 NextLevelButton 直接隐藏，仅保留重新开始/返回选关/返回主菜单/退出四条出口。MainMenuButton 无论胜负均可见，点击时按 Paused=false → SceneManager.LoadMainMenu() 顺序回到 Meta Loop 起点，CurrentLevelIndex 被 LoadMainMenu 重置为 0，完成关卡结束→主菜单的反向闭环。NextLevelButton 点击前会校验 _lastIsVictory 内部状态防止战败误触发，随后按 Paused=false → SceneManager.LoadNextLevel 的顺序切入下一关，或在无下一关时回落至选关界面，形成"主菜单 → 选关 → 通关 → 解锁 → 下一关 → 返回选关 / 返回主菜单"的完整局外 Meta Loop 架构闭环，解锁进度通过 ConfigFile 写入用户目录在多次启动间持久化留存。
